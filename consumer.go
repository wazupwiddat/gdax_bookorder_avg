...
// FileCheckPoint checkpoints to a file in application directory
type FileCheckPoint struct{}

// Get ...
func (f FileCheckPoint) Get(shardID string) (string, error) {
	filename := fmt.Sprintf("checkpoint-%s", shardID)
	ok, _ := exists(filename)
	if !ok {
		return "", nil
	}
	data, err := ioutil.ReadFile(filename)
	if err != nil {
		log.Println("Checkpoint Get:", err)
		return "", err
	}
...
// Set ...
func (f FileCheckPoint) Set(shardID string, sequenceNumber string) error {
	if err := ioutil.WriteFile(fmt.Sprintf("checkpoint-%s", shardID), []byte(sequenceNumber), 0644); err != nil {
		log.Println("Checkpoint Set:", err)
		return err
	}
	return nil
}
...
// NewConsumer creates a kinesis consumer with default settings. Use Option to override
